local loadstring = function(...)
	...
	if err and nova then nova:CreateNotification('Nova', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/UdAxol/novabws/'..readfile('nova/profiles/commit.txt')..'/'..select(1, path:gsub('nova/', '')), true) end)
		if not suc or res == '404: Not Found' then error(res) end
		if path:find('.lua') then res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after nova updates.\n'..res end
		writefile(path, res)
	end
...
local vm = loadstring(downloadFile('nova/libraries/vm.lua'), 'vm')()
...
run(function()
		...
		for _, scr in scripts do
			...
			for _, proto in deserializedcode.protoList do
				...
				for i, inst in code do
					if inst.opcode == 4 then
						stack[inst.A] = inst.D
					elseif inst.opcode == 5 then
						stack[inst.A] = inst.K
					elseif inst.opcode == 6 then
						stack[inst.A] = stack[inst.B]
					elseif inst.opcode == 12 then
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or nova.Loaded == nil
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Nova', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
...
run(function()
	...
	Wallbang = nova.Categories.Combat:CreateModule({
		Name = 'Wallbang',
		Function = function(callback)
			if callback then
				local hook
...
run(function()
	...
	AutoArrest = nova.Categories.Blatant:CreateModule({
		Name = 'AutoArrest',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoPop = nova.Categories.Blatant:CreateModule({
		Name = 'AutoPop',
		Function = function(callback)
			if callback then
				task.spawn(function()
...
run(function()
	...
	Punch = nova.Categories.Blatant:CreateModule({
		Name = 'AutoPunch',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTaze = nova.Categories.Blatant:CreateModule({
		Name = 'AutoTaze',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	LazerGodmode = nova.Categories.Blatant:CreateModule({Name = 'LazerGodmode'})
end)
...
run(function()
	...
	InfNitro = nova.Categories.Utility:CreateModule({
		Name = 'InfiniteNitro',
		Function = function(callback)
			if callback then
