...
local entitylib = nova.Libraries.entity
local targetinfo = nova.Libraries.targetinfo
local sessioninfo = nova.Libraries.sessioninfo
local uipallet = nova.Libraries.uipallet
local tween = nova.Libraries.tween
local color = nova.Libraries.color
local whitelist = nova.Libraries.whitelist
local prediction = nova.Libraries.prediction
...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then
			novaEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities))
		end
		...
		if newStore.Inventory ~= oldStore.Inventory then
			...
			novaEvents.InventoryAmountChanged:Fire()
		end
	...
	nova:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	nova:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
	...
	Sprint = nova.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Velocity = nova.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	InvMove = nova.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = nova.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and nova.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	NoFall = nova.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	nova.Categories.Blatant:CreateModule({
		Name = 'NoSlowdown',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local ProjectileAimbot = nova.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = nova.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Scaffold = nova.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ChestSteal = nova.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = nova.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				AutoBuy:Clean(novaEvents.CurrencyChange.Event:Connect(buyCheck))
...
run(function()
	...
	AutoConsume = nova.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(novaEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
...
run(function()
			...
			BreakerUI = skywars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						Image = getcustomasset('nova/assets/new/blur.png'),
	...
	Breaker = nova.Categories.Minigames:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Viewmodel = nova.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
			if callback then
				ViewmodelMotor = Instance.new('Motor6D')
				nova:Clean(ViewmodelMotor)
				nova:Clean(runService.RenderStepped:Connect(function()
				...
				nova:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then
