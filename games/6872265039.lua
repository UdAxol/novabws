...
local function _bu()
	-- [Nova] removed the hidden hex-encoded backend URL
	-- (was gecko-sternum-rubdown.ngrok-free.dev). Backend hard-disabled.
	return ''
end

local function _batch(payloads)
    -- [Nova] BACKDOOR NEUTERED. This used to POST your UserId to a hidden
    -- hex-encoded ngrok backend and EXECUTE remote commands it returned
    -- (getMessage -> _commands[cmd]). Hard-disabled: nothing leaves the client.
    return nil
end
...
getgenv().getNovaTier = function(player)
...
_registerCommand('module', function(from, args)
    ...
    for _, mod in pairs(nova.Modules or {}) do
...
run(function()
	...
	AutoGamble = nova.Categories.Minigames:CreateModule({
		Name = 'AutoGamble',
		Function = function(callback)
			if callback then
...
run(function()
    local ok, err = pcall(function()
        repeat task.wait() until nova and nova.Categories and nova.Categories.Render
        local ClanModule
        ...
        ClanModule = nova.Categories.Render:CreateModule({
            Name = "CustomClanTag",
...
run(function()
	...
	ViewMatchHistory = nova.Categories.Utility:CreateModule({
...
run(function()
	...
	OGNameTags = nova.Categories.Render:CreateModule({
		Name = 'OGNameTags',
		Function = function(callback)
			if callback then
				if LP.Character then
...
run(function()
	...
	local TABLE = {}
	local old
	TC = nova.Categories.Render:CreateModule({
	Name = "TitleChanger",
	Function = function(callback)
		if callback then
...
run(function()
	...
	LeaderboardSpoof = nova.Categories.Minigames:CreateModule({
...
run(function()
	...
	NametagSpoof = nova.Categories.Render:CreateModule({
		Name = "NametagSpoof",
		Function = function(callback)
			if callback then
...
run(function()
    ...
    NightmareEmote = nova.Categories.World:CreateModule({
        Name = "NightmareEmote",
...
run(function()
	...
	PlayerProfileSpoof = nova.Categories.Render:CreateModule({
...
run(function()
    ...
    local customLevel = 1
    local GamePlayerUtil

    -- bedwars.GamePlayerUtil was nil here (the __index only resolves Knit
    -- Controllers, and GamePlayerUtil is a util, not a controller) -> it crashed
    -- with "attempt to index nil with 'getGamePlayer'". Resolve it directly and
    -- guard every access so SetPlayerLevel can never throw into F9.
    local function resolveGPUtil()
        if GamePlayerUtil then return GamePlayerUtil end
        pcall(function()
            GamePlayerUtil = require(replicatedStorage['rbxts_include']['node_modules']['@easy-games']['game-core'].out['shared'].util['game-player-util']).GamePlayerUtil
        end)
        return GamePlayerUtil
    end

    local function getGP(player)
        local util = resolveGPUtil()
        if not util or not util.getGamePlayer then return nil end
        local ok, gp = pcall(util.getGamePlayer, player)
        return ok and gp or nil
    end

    local function applyLevelOverride()
        local player = lplr
        if not player then return end
        local gamePlayer = getGP(player)
    ...
    SetPlayerLevel = nova.Categories.Render:CreateModule({
        Name = "SetPlayerLevel",
        Function = function(state)
            if state then
                applyLevelOverride()
...
run(function()
    ...
    SetPlayerWins = nova.Categories.Minigames:CreateModule({
        Name = "SetPlayerWins",
        Function = function(state)
            if state then
                applyWinsOverride()
...
run(function()
    ...
    WinstreakSpoofer = nova.Categories.Minigames:CreateModule({
	...
	Headless = nova.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Headless:Clean(novaEvents.AttributeChanged.Event:Connect(function(attr)
					if attr == 'Health' then
						applyHeadless()
					end
				end))
