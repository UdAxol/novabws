...
local tween = nova.Libraries.tween
local targetinfo = nova.Libraries.targetinfo
local getfontsize = nova.Libraries.getfontsize
local getcustomasset = nova.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('nova/assets/new/blur.png')
...
local function isFriend(plr, recolor)
	if nova.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(nova.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and nova.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(nova.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not nova.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function notif(...)
	return nova:CreateNotification(...)
end
...
local function serverHop(pointer, filter)
	visited = shared.novaserverhoplist and shared.novaserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Nova', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Nova', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Nova', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Nova', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

nova:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.novaserverhoplist = '"..table.concat(visited, '/').."'\nshared.novaserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('nova/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('nova/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('nova/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
nova.Libraries.entity = entitylib
nova.Libraries.whitelist = whitelist
nova.Libraries.prediction = prediction
nova.Libraries.hash = hash
nova.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if nova.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and nova.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(nova.Categories.Friends.Options['Friends color'].Hue, nova.Categories.Friends.Options['Friends color'].Sat, nova.Categories.Friends.Options['Friends color'].Value)
		end
	...
	nova:Clean(nova.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	nova:Clean(nova.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	nova:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	nova:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = nova.Uninject
				nova.Uninject = function()
					notif('Nova', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('Nova', plr.Name..' is using nova!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				nova:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				nova:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		local suc = pcall(function()
			local _, subbed = pcall(function()
				return game:HttpGet('https://github.com/UdAxol/whitelistcheck')
			end)
			local commit = subbed:find('currentOid')
			commit = commit and subbed:sub(commit + 13, commit + 52) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/UdAxol/whitelistcheck/'..commit..'/PlayerWhitelist.json', true)
		end)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('nova/profiles/whitelist.json') and readfile('nova/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				nova:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and nova.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement and whitelist.data.Announcement.expiretime and whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'NOVA ANNOUNCEMENT: '..whitelist.data.Announcement.text
				...
				pcall(function()
					writefile('nova/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillNova then
				nova:Uninject()
				return true
			end
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = nova.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
...
run(function()
	...
	AimAssist = nova.Categories.Combat:CreateModule({
		Name = 'AimAssist',
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not nova.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = nova.gui.AbsoluteSize / 2
...
run(function()
	...
	AutoClicker = nova.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not nova.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = nova.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SilentAim = nova.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	TriggerBot = nova.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiFall = nova.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Fly = nova.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = nova.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = nova.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = nova.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	Killaura = nova.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	LongJump = nova.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = nova.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = nova.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speed = nova.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = nova.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = nova.Categories.Blatant:CreateModule({
		Name = 'SpinBot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = nova.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = nova.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = nova.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = nova.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('nova/assets/new/arrowmodule.png')
	...
	Arrows = nova.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = nova.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	local ESPLoop = {
		Drawing2D = function()
			for ent, EntityESP in Reference do
				...
				if shared.nova and shared.nova.hackerTable and table.find(shared.nova.hackerTable, ent.Player) and entitylib.isAlive then
	...
	ESP = nova.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = nova.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if nova.ThreadFix then
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = nova.Modules.Fly and nova.Modules.Fly.Enabled or nova.Modules.LongJump and nova.Modules.LongJump.Enabled or nova.Modules.InfiniteFly and nova.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = nova.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = nova.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = nova.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Radar = nova:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('nova/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = nova.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = nova:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.novasessioninfo = '"..httpService:JSONEncode(nova.Libraries.sessioninfo.Objects).."'")
					end
				end))

				if shared.novasessioninfo then
					for i, v in httpService:JSONDecode(shared.novasessioninfo) do
						if nova.Libraries.sessioninfo.Objects[i] and v.Saved then
							nova.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end

				repeat
					if nova.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end

						for i, v in nova.Libraries.sessioninfo.Objects do
	...
	nova:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if nova.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (nova.gui.AbsoluteSize.X / 2)
	...
	nova.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = nova.gui.AbsoluteSize
	...
	Tracers = nova.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(nova.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = nova.gui

	Waypoints = nova.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = nova.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = nova.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = nova.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = nova.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = nova.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'nova on top')
...
run(function()
	...
	Disabler = nova.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	nova.Categories.Utility:CreateModule({
		Name = 'Panic',
		Function = function(callback)
			if callback then
				for _, v in nova.Modules do
					if v.Enabled then
						v:Toggle()
					end
				end
...
run(function()
	...
	Rejoin = nova.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = nova.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.novaserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.novaserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.novaserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not nova.Loaded then
			repeat task.wait() until nova.Loaded
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				nova.Save = function() end
				if nova.Profile ~= Profile.Value then
					nova.Profile = Profile.Value
					nova:Load(true, Profile.Value)
				end
	...
	StaffDetector = nova.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	nova.Categories.World:CreateModule({
		Name = 'AntiAFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = nova.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = nova.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = nova.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then
				local oldfloor
...
run(function()
	...
	Xray = nova.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = nova.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = nova.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = nova.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChinaHat = nova.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if nova.ThreadFix then
...
run(function()
	...
	Disguise = nova.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = nova.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = nova.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = nova.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = nova.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = nova.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
...
run(function()
    ...
    local InstantPP = nova.Categories.Utility:CreateModule({
        Name = 'InstantPP',
        Function = function(callback)
            if callback then
...
run(function()
    if not setfflag or type(setfflag) ~= 'function' then
        nova:CreateNotification('Nova', 'setfflag not supported by this executor', 5, 'warning')
        return
    end
    ...
    local function ChangeFFlag(suc)
        ...
        if not success or typeof(json) ~= 'table' then
            nova:CreateNotification('Nova', 'Invalid json format for fflag', 12, 'warning')
            return
        end
        ...
        nova:CreateNotification('Nova', 'FFlags applied, Go in a new game to take effect', 12, 'info')
    end

    FFlag = nova.Legit:CreateModule({
        Name = 'FFlag Editor',
        Function = function(call)
            if call then
                ChangeFFlag(true)
            else
                nova:CreateNotification('Nova', 'In order to disable fflags you have applied, you need to restart Roblox', 20, 'info')
            end
        end
    })
...
run(function()
	...
	Shaders = nova.Legit:CreateModule({
		Name = "Shaders",
		Function = function(callback)
