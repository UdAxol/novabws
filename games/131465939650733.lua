...
nova.Place = 5938036553
if isfile('nova/games/'..nova.Place..'.lua') then
	loadstring(readfile('nova/games/'..nova.Place..'.lua'), 'bedwars')()
else
	if not shared.NovaDeveloper then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/UdAxol/novabws/'..readfile('nova/profiles/commit.txt')..'/games/'..nova.Place..'.lua', true)
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('nova/games/'..nova.Place..'.lua'), 'bedwars')()
		end
	end
end
