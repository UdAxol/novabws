...
run(function()
	...
	task.spawn(function()
		...
		if map and nova.Loaded ~= nil then
			nova:Clean(map.DescendantAdded:Connect(function(v)
				parsePositions(v, function(pos)
					store.blocks[pos] = v
				end)
			end))
			nova:Clean(map.DescendantRemoving:Connect(function(v)
				parsePositions(v, function(pos)
...
run(function()
	...
	nova.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = nova.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
