...
local drawingactor = loadstring(downloadFile('nova/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		local oldreload = shared.novareload
		nova.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.nova
			local executionString = "loadfile('nova/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.novareload = true\n'..executionString
			end
			...
			notif('Nova', 'Failed to find actor', 10, 'alert')
		end)
	else
		nova.Load = function()
			notif('Nova', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('Nova', 'Failed to hook ('..id..')', 10, 'alert')
	end
...
run(function()
	...
	if nova.Loaded == nil then return end
	...
	nova:Clean(Drawing.kill or function() end)
	nova:Clean(function()
...
run(function()
	...
	GrenadeTP = nova.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	GunModifications = nova.Categories.Blatant:CreateModule({
...
run(function()
	...
	GrenadeESP = nova.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = nova.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = nova.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = nova.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
...
run(function()
	...
	PickupRange = nova.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BulletTracers = nova.Legit:CreateModule({
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then
