...
local function _wallSampleClear(origin, sample, ignoreobject)
	local hit = workspace.Raycast(workspace, origin, sample - origin, ignoreobject)
	if not hit then return true end
	local inst = hit.Instance
	if not inst then return true end
	if inst.Transparency and inst.Transparency >= 0.5 then return true end
...
entitylib.Wallcheck = function(origin, position, ignoreobject)
	...
	if _wallSampleClear(origin, position, ignoreobject) then return nil end
	if _wallSampleClear(origin, position + Vector3.new(0, 2, 0), ignoreobject) then return nil end
	if _wallSampleClear(origin, position + Vector3.new(0, -1.5, 0), ignoreobject) then return nil end
