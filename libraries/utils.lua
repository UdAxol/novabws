...
functions.GetTarget = function(args: table?): table
    ...
    for i,v in players:GetPlayers() do
        if shared.rendernova and not RenderLibrary.whitelist:get(2, v) then
            continue
        end;
...
functions.isEnabled = function(button: string, category: string?): boolean
	local success, enabled = pcall(function()
		return shared.rendernova.ObjectsThatCanBeSaved[button..(category or 'OptionsButton')].Api.Enabled
	end)
...
functions.GetAllTargets = function(distance: number?, mobs: boolean?, sort: (table, table) -> (boolean?))
	...
	for i,v in players:GetPlayers() do
		if v ~= lplr and isAlive(v, game.PlaceId == 11630038968) and isAlive(lplr, true) then
			...
			if shared.novaentity and not shared.novaentity.isPlayerTargetable(v) then
				continue
			end;
...
functions.warningNotification = function(title: string, text: string, delay: number): Frame
	local suc, res = pcall(function()
		local color = shared.rendernova.ObjectsThatCanBeSaved['Gui ColorSliderColor'].Api
		local frame = shared.rendernova.CreateNotification(title, text, delay, 'assets/WarningNotification.png')
...
functions.InfoNotification = function(title: string, text: string, delay: number): Frame
	local success, frame = pcall(function()
		return shared.rendernova.CreateNotification(title, text, delay)
	end)
	return success and frame
end;

functions.isflying = function()
    if shared.rendernova == nil then return false end
    for i,v in shared.rendernova.ObjectsThatCanBeSaved do
...
functions.errorNotification = function(title: string, text: string, delay: number): Frame
	local success, frame = pcall(function()
		local notification = shared.rendernova.CreateNotification(title, text, delay or 6.5, 'assets/WarningNotification.png')
...
functions.homefolder = function(): string
    for i,v in ({'nova/', 'nova/Render', 'nova/Render/lib', 'nova/Render/scripts'}) do
